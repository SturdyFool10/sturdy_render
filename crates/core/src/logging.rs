...
pub fn make_log_filter() -> EnvFilter {
    if cfg!(debug_assertions) {
        // Debug: allow all logs
        EnvFilter::new("trace,wgpu=warn,wgpu::*=warn,naga=warn,naga::*=warn")
    } else {
        // Release: only allow logs from core and things that depend on core
        // (You can add more crate/module names as needed)
        EnvFilter::new("core=info,core::*=info")
    }
}
...
pub fn init_logging() {
    ...
    let mut filter = make_log_filter();
